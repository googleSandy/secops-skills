...
### Latency and limits
Processing and visualization delays in Google SecOps are subject to the following architectural limits:  Search visibility: 2–15 minutes after ingestion. Rule execution: 5–10 minutes after event arrival. UI visualization: High-volume log data is subject to a 10,000-row visualization limit to maintain browser performance.  Caution: A "sync failure" is often reported when a change does not immediately reflect in the UI. If the event occurred less than 15 minutes ago for search or 10 minutes ago for rules, it is still within the standard processing pipeline and may not indicate a system failure.