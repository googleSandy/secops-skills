...
## Ingest and process data in Google SecOps
...
Threat intelligence: Google SecOps automatically compares event data against Google's vast threat intelligence, including sources like Google Threat Intelligence and Safe Browsing, to identify known malicious threats, like `domains`, `IP addresses`, and `file hashes`.
...
### UDM search
UDM search is the primary and fastest search method, used for most investigations.  What it searches: It queries the normalized and indexed UDM events. Because all data is parsed into this standard format, you can write one query to find the same activity (like a login) across all your different products (for example, Windows, Okta, Linux). How it works: You use a specific syntax to query fields, operators, and values.
Example: `principal.hostname = "win-server" AND target.ip = "10.1.2.3"`
Results are typically available within 2–15 minutes of ingestion.
### Raw log search
Use Raw log search to find something in the original, unparsed log message that may not have been mapped to a UDM field. This search method is optimized for high-velocity hunts, typically returning results in under 2 seconds for specific indicators such as file hashes or IP addresses.  What it searches: It scans the original, raw text of the logs before they were parsed and normalized. This is useful for finding specific strings, command-line arguments, or other artifacts that aren't indexed UDM fields. How it works: You use the `raw =` prefix. It can be slower than UDM search because it doesn't search indexed fields. Example (String): `raw = "PsExec.exe"` Example (Regex): `raw = /admin\$/`
### Stats search
Use Stats search for long-term trending that aggregates millions of rows of data. Because the platform must perform statistical analysis and grouping, expect longer load times for these queries.
...
End-to-end data availability is the total time between when an event occurs and when it's available for search or rule execution in Google SecOps. This latency is the sum of the following two components:
Source-side availability delay: The time between when an event occurs and when the source system makes the log data available for ingestion. This delay depends on the source system's architecture, processing, batching, and API publication schedules. Google SecOps cannot influence this delay. Examples include delays when a system writes logs to a storage bucket or publishes them to an API endpoint.
Google SecOps processing time: The time Google SecOps requires to process the data after receiving it. This duration includes internal pipeline stages such as ingestion, parsing, normalization, indexing, and enrichment.
You must consider both components when troubleshooting data visibility timelines.
### Delays originating from the data source
The following factors can influence the source-side availability delay:  Batch processing: Some systems generate logs in batches at set intervals (for example, hourly). API latency: The source API can have inherent delays in making new events queryable. Event creation and publication time: An event timestamp in a log might be much earlier than the timestamp when the log is finalized and becomes available for collection. Throttling: Source-side API rate limits can slow data retrieval. Initial backfill: Serving and ingesting large volumes of historical data takes time.
These delays vary by data source and log type. For more details on ingestion methods, see Data ingestion overview. The Feed management API reference describes specific considerations for log types such as Microsoft Graph, SentinelOne, Okta, and CrowdStrike.
### Google SecOps processing time
...
### Latency and limits
Processing and visualization delays within the Google SecOps platform are subject to the following architectural limits after Google SecOps receives the data:  Search visibility: 2 to 15 minutes after ingestion. Rule execution: 5 to 10 minutes after event arrival. UI visualization: To maintain browser performance, high-volume log data is subject to a 10,000-row visualization limit.  Note: These times start after Google SecOps receives the data. End-to-end data availability also depends on delays originating from the data source that occur before the data enters the Google SecOps pipeline.Caution: If a change doesn't immediately appear in the UI, it doesn't necessarily indicate a sync failure. If the event occurred less than 15 minutes ago for search or 10 minutes ago for rules, the data is likely still in the standard processing pipeline. The delay doesn't indicate a system failure.