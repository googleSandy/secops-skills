...
Whereas, the following example is a valid search: `(metadata.event_type = "NETWORK_CONNECTION"` or `metadata.event_type = "NETWORK_DHCP")` Note: To search for multiple enumerated values, you must explicitly list each value with the `OR` operator." See the preceding query for example.
...
To search UDM events for ingested logs with older event timestamps, you can use All time option to search and query over `metadata.ingested_timestamp`.  Note: It's not possible to map the ingestion time with the search time range.
...
## Troubleshooting
If you receive a generic error message, such as "Error: Search has encountered an error and couldn't load data", take the following steps to resolve the issue. If the error persists, contact Customer Support.  Connect to Google SecOps from a different network. For example, use a cloud VM to help identify any network issues. Make sure Chronicle API calls are allowed by your organization's firewall or proxy configuration or policy. Verify that no data limit is configured for Search API calls, as searches can return large datasets. Search queries can run asynchronously and might require more time to return data. You can view previously run searches in the search history and select them to view the results later.