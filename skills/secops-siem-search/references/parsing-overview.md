...
Google SecOps can receive log data originating from the following ingestion sources:  Google SecOps forwarder Chronicle API Feed Chronicle API Third-party technology partner
...
The `metadata.event_type` in a UDM Event record is not the same as the log_type defined when ingesting data using the Chronicle API. These two attributes store different information.
